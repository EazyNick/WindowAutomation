...
try:
    ...
    from module import ark_trivetower_run
...
class ArkCommand(Command):
    ...
    """
    def execute(self):
        log_manager.logger.info("방주 실행 시작")
        try:
            ark_trivetower_run()
            log_manager.logger.info("방주 실행 완료")
        except Exception as e:
            log_manager.logger.error(f"방주 실행 중 오류 발생: {e}")

if __name__ == "__main__":
    log_manager.logger.info("ArkCommand 테스트 시작")
    ...
    log_manager.logger.info("ArkCommand 테스트 종료")
