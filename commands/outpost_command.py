...
class OutpostCommand(Command):
    ...
    """
    def execute(self):
        log_manager.logger.info("일일 전투 보상 실행 시작")
        try:
            outpost_run()
            log_manager.logger.info("일일 전투 보상 실행 완료")
        except Exception as e:
            log_manager.logger.error(f"일일 전투 보상 실행 중 오류 발생: {e}")

if __name__ == "__main__":
    log_manager.logger.info("OutpostCommand 테스트 시작")
    ...
    log_manager.logger.info("OutpostCommand 테스트 종료")
